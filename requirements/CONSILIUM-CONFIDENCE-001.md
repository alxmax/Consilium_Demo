...
- The dissent (−0.10) and abstain (−0.15) penalties are applied only when the Sentinel personality is the dissenter/abstainer. Essentialist and Verifier dissent/abstain carry no penalty because Sentinel (conservative-leaning, K=0.40) is the sole risk-anchor voice; its absence from the majority signals a materially higher risk that the base vote-pattern score does not capture.
...
- Given `vote_pattern='2-1'` with a dissent list containing the Sentinel personality, the output confidence equals `VOTE_PATTERN_CONFIDENCE['2-1']` minus the `SENTINEL_DISSENT_PENALTY` (0.10), rounded to 3 decimal places.
