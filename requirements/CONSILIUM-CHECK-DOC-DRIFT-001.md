...
- When multiple invariants are violated, all violations are collected across all six check families before any output; the script prints every violation (numbered) to stderr then exits 1 — it never stops at the first failure.
