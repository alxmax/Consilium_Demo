...
## WHAT — Contract
- Shall provide a deterministic registry of exactly three Trias personalities — Essentialist, Verifier, Sentinel — each with a `name`, a `weights` dict (`generator`, `control`, `conservator` summing to 1.0), and a `lens` path.
...
- Running with `--name essentialist` emits a single JSON object matching the Essentialist entry; the returned dict is a deep copy and mutations do not affect subsequent calls.
