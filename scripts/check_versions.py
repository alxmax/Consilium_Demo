"""Manifest version-coherence gate (CI tooling — not part of the deliberation engine).

The plugin's semver lives in two static manifests that no single process can derive
at load time (Claude Code reads plugin.json / marketplace.json as data; neither can
import the other). This script is the single point that asserts they agree, so
drift fails the build instead of shipping silently. Mirrors requirement-manager's
scripts/check_versions.py (same mechanism, adopted 2026-07-06).
...
Usage:
    ...
    python scripts/check_versions.py --fix   # propagate plugin.json's version into marketplace.json
"""
# implements: CONSILIUM-CHECK-VERSIONS-001
...
def _fix(canonical: str) -> int:
    ...
    occurrence, so a bump is one edit + one command instead of two hand-edits.
