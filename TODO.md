...
- [x] **`.consilium/` data folder** — relocated `runs/` + `FEEDBACK.html` + the `.run_path_map.json` sidecar under a single gitignored `.consilium/` dir (2026-05-26). Paths centralized in `scripts/utils.py` (`DATA_DIR`/`RUNS_DIR`/`FEEDBACK_PATH` + `canonical_run_path`); ~11 scripts repointed to the constants; `runs/README.md` → `docs/runs-schema.md`. Run-path keys stay `runs/<file>.json` (relative to `.consilium/`), so the 175 existing sidecar/FEEDBACK entries needed **no migration**; `canonical_run_path` lets `--run-path` accept any spelling. Verified: priors / efficiency --by-mode / log_feedback round-trip / test_feedback_html (11/11) / run_evals. **Follow-up:** `benchmark/run_task.py` regex `\bruns/[\w.-]+\.json\b` still scans skill stdout — the orchestrator now prints `.consilium/runs/<f>.json`; confirm the benchmark PEND-conversion still matches (it matches the `runs/` substring, but verify) or update the regex.
...
- [ ] **Move feedback artifacts into `.consilium/` (PLANNED — resume next session).** Deliberated 2026-05-26 (`runs/2026-05-26_0100_consilium-data-folder.json`, chosen `centralize_then_move`, conf 0.72). Plan: add `DATA_DIR`/`RUNS_DIR`/`FEEDBACK_PATH`/`RUN_PATH_MAP` to `scripts/utils.py`; repoint all ~18 scripts to import them; `git mv runs/ → .consilium/runs/` and `FEEDBACK.html → .consilium/FEEDBACK.html`; update `.gitignore` + SKILL.md/CLAUDE.md/modes docs. **Decisions made:** runs/README.md → **move to `docs/runs-schema.md`**; benchmark/run_task.py regex coupling (`\bruns/...json\b`) → **follow-up TODO** (below), not in this change. **Edge cases to honour:** the `run_path_map` sidecar + `efficiency.py` outcome-join hardcode `runs/<f>.json` strings — repoint both. Goal is local-tidiness only (`.gitignore` already hides these publicly).
- [ ] **Follow-up to `.consilium/` move:** update `benchmark/run_task.py` PEND-conversion regex from `\bruns/[\w.-]+\.json\b` to `.consilium/runs/...` (else benchmark PEND→PEND_HEADLESS conversion silently stops once the skill writes to the new path).
