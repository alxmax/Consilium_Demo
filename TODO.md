...
- [x] **Validation regression — spec-priming leak**: rewrite the empirical-motivation prose in modes/dialectic.md + SKILL.md so the validation task's correct letter is NOT embedded in spec text the orchestrator reads. Replace "correct answer is B (drive)" with a content-free reference (e.g., "see runs/senate/.../validation-task-01"). Then re-run for clean validation. **Done 2026-05-28**: both files now reference `experiments/dialectic-skeptic-on-scale-down-validation-2026-05-28.md` instead of spelling out the answer.
- [ ] Construct a NEW eval task that exercises Skeptic-on-scale_down without spec text revealing the answer. Goal: empirically distinguish (a) Skeptic actually catching constraints from (b) spec-priming. Until this exists, the fix is unvalidated.
