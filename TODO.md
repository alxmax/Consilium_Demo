...
- [ ] **Efficiency / model-count audit** — how many sub-agents are actually needed (1/2/3)? vary by complexity? define the target end-state.
  ...
  - **Target end-state:** a diff-checkable change to the SKILL.md Routing Boundary table (not a prose claim).
  - **Precondition:** Trias-vs-Sequential paired corpus n≥5, same spec both arms, oracle-validated. Current n=6 (pipeline-bench R1+R2) is insufficient — all algorithmic tasks. Revisit when benchmark reaches n≥20 across diverse task types (architectural deliberations included).
  - *(Consilium deliberation 2026-05-26: `runs/2026-05-26_1200_p1-efficiency-p2-explore-commit.json`, conf=0.74)*
- [x] **EXPLORE → PLAN → CODE → COMMIT workflow** — formalize; relates to the post-deliberation implementation pipeline above. *(Closed as docs-only: EXPLORE/COMMIT labels added to SKILL.md OTAL section, no new Steps — see 2026-05-26 deliberation above.)*
