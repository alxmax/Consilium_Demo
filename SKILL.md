...
8. **Verdictul e advisory** — user-ul decide:
   ...
   - `DEEPLY_SPLIT` (quorum atins, fără majoritate, dar ambele facțiuni GO și STOP ating `POLARIZATION_THRESHOLD=3` — pattern 4-3, 3-3-1, 3-4) → advisory ca `UNREACHABLE`: orchestratorul trebuie să escaladeze la user cu vote matrix + `modify_requests` per senator, fără acțiune automată
   - `MODIFY` (default — splits ne-polarizate sau cu absentări) → două căi posibile: **accept** (default — tratează `modify_requests` ca TODO advisory și procedezi cu propunerea originală; preservă status quo) sau **R2** (re-rulează Senate cu o propunere revizuită care adresează `modify_requests`-urile senatorilor). Dacă alegi R2 și verdictul rămâne `MODIFY` după 3 cicluri consecutive pe aceeași propunere, orchestratorul afișează un avertisment soft: "3 cicluri R2 fără schimbare de verdict — consideră accept sau descompune propunerea în bucăți mai mici."
...
```
Suita rulează: prompt structure, fixture, verdict GO unanimous/quorum, MODIFY default, UNREACHABLE, unrecognized-vote, **multi-round position change (Law 2+4)**, **cross-questions violation (Law 2)**, **blocaj pending + blocaj resolution (Law 3)**, bundle persistence, collision-safe write. Toate 18 trebuie PASS înainte de commit pe `senate_synth.py` sau orice `prompts/senators/*.md`.
