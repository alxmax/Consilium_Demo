...
# requirements/_map.html is a self-contained React app generated by
# `reqmap.py sync`. It lives outside docs/ so GitHub Pages can't serve it
# directly; copy it into docs/ so the architecture button link works.
REQMAP_SRC = REPO / "requirements" / "_map.html"
REQMAP_DST = DOCS / "requirements-map.html"
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Copy requirements map into docs/ so GitHub Pages can serve it.
    if REQMAP_SRC.exists():
        reqmap_content = REQMAP_SRC.read_text(encoding="utf-8")
        if args.check:
            current = REQMAP_DST.read_text(encoding="utf-8") if REQMAP_DST.exists() else None
            if current != reqmap_content:
                stale.append(REQMAP_DST.name)
        else:
            REQMAP_DST.write_text(reqmap_content, encoding="utf-8", newline="\n")
            print(f"wrote {REQMAP_DST.relative_to(DOCS.parent)} ({len(reqmap_content):,} bytes)")
