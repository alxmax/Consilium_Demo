...
const MODES = [
  ...
  },
  {
    ...
    use: 'code changes with non-obvious implementation + verification strategy',
    cost: '1.33×',
  ...
  },
  {
    ...
    name: 'Trias',
    tag: 'OPT-IN',
    plain: 'Three independent Claude sub-agents run the full Sequential pipeline in parallel. Each has a different personality (bold / structural / protective). Their three answers go to a democratic vote.',
    desc: 'Three sub-agents dispatched in parallel, one per personality lens (Pioneer / Architect / Steward). Each runs Sequential internally with its lens prepended to the system prompt. Each returns a chosen answer. A simple majority vote decides the winner — 3-0 / 2-1 / 2-0.',
  ...
  },
  {
    ...
    name: 'Parallel — auto',
    tag: 'AUTO',
    plain: 'Not user-selectable. The system fires this automatically when a change is both critical and irreversible. Three sub-agents (one per voice) run in true parallel, each in its own context window.',
    desc: 'Auto-triggered when magnitude=critical AND reversibility=irreversible. Generator, Control, and Conservator run as three independent sub-agents in parallel — each with its own context window, no cross-contamination. Also runs as a silent audit every 20 deliberations to detect divergence from Sequential.',
    use: 'auto-only · critical + irreversible changes · silent audit every 20 runs',
    cost: '3× (3 sub-agents)',
    isolation: 'full context isolation per voice',
    voices: 'Cons + Gen + Ctrl in parallel',
    subagents: '3 (1 per voice)',
  },
];
...
const WALKTHROUGHS = {
  seq_blind: {
    ...
    steps: [
      ...
      { id: '3', name: 'strip→gen', caption: 'Strip: only {magnitude, counterparty_risks, tokens_budget} reach the Generator. The rest stays in main context.',
  ...
  trias: {
    ...
    steps: [
      ...
      { id: '2', name: 'dispatch', caption: 'Three sub-agents dispatched in parallel — one per personality. Each gets a different lens prepended over the core voices.',
  ...
  parallel_auto: {
    ...
    steps: [
      ...
      { id: '2', name: 'dispatch', caption: 'Three sub-agents dispatched simultaneously — one per voice. Each gets its own context window, no shared memory.',
        nodes: { orch: 'done', gate: 'done', cons: 'active', gen: 'active', ctl: 'active', agg: 'idle' }, arrows: ['orch_cons', 'orch_gen', 'orch_ctl'] },
      { id: '3', name: 'parallel work', caption: 'All three voices think at the same time. No voice can see the others until they return. True independence.',
        nodes: { orch: 'done', gate: 'done', cons: 'active', gen: 'active', ctl: 'active', agg: 'idle' }, arrows: ['orch_cons', 'orch_gen', 'orch_ctl'] },
      { id: '4', name: 'collect', caption: 'Each sub-agent returns its final JSON to the orchestrator. No intermediate reasoning passes back.',
        nodes: { orch: 'done', gate: 'done', cons: 'done', gen: 'done', ctl: 'done', agg: 'active' }, arrows: ['orch_cons', 'orch_gen', 'orch_ctl', 'cons_agg', 'gen_agg', 'ctl_agg'] },
      { id: '5', name: 'aggregate', caption: 'Aggregator applies the cascade and conservative_override scheme over the three independent outputs.',
        nodes: { orch: 'done', gate: 'done', cons: 'done', gen: 'done', ctl: 'done', agg: 'active' }, arrows: ['orch_cons', 'orch_gen', 'orch_ctl', 'cons_agg', 'gen_agg', 'ctl_agg'] },
      { id: '6', name: 'done', caption: 'Winner chosen. Also fires as a silent audit every 20 deliberations to check sequential mode hasn\'t drifted.',
