...
const MODES = [
  ...
  },
  {
    ...
    name: 'Dialectic',
    tag: 'OPT-IN',
    plain: 'A normal Sequential pass (itself now 1 dispatched sub-agent), then a single Skeptic sub-agent is dispatched specifically to challenge the chosen answer. The mode is that focal post-hoc challenge — code context is injected when the change is code, but writing the code itself is the Integration step.',
    desc: 'Stage 1: a normal Sequential pass — 1 dispatched sub-agent (when the change is code, voice inputs are enriched with language / framework / files_touched[] / test_files[] / ci_gates context). Stage 2: a single Skeptic sub-agent receives only the chosen answer. Skeptic runs unconditionally — not gated on the confidence band. Optional lens (opt-in, default OFF): --lens essentialist puts the Essentialist lens on the decider (the Sequential voices) and tints the Skeptic with the Verifier lens (via the Verifier lens\'s Skeptic carve-out) — complementary roles, not a vote; decider ≠ skeptic is enforced by validate_report.py through telemetry.lens_applied. Off by default (lens text is not free) and value-unproven.',
    use: 'any chosen answer that benefits from a focal post-hoc challenge before it ships',
    cost: '1.33×',
    isolation: 'Sequential\'s own dispatch + 1 isolated Skeptic',
    voices: 'Gen · Cons · Ctrl + Skeptic',
    subagents: '2 (Sequential + Skeptic)',
  },
  {
    id: 'trias',
    name: 'Trias',
